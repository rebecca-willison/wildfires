...
######################################################################
library(reticulate)
work_dir <- '/home/Rebecca/Documents/git-repos/wildfires'
source_python(paste(work_dir,'data_prep/src/gee_weather_summary.py',sep ='/'))
# data wrangling
library(dplyr)
library(readr)
library(tidyr)
...
fires <- read_csv(paste(work_dir,'data/ca_modeling_dataset.csv',sep='/')) %>% 
  data.frame %>% 
...
for(i in 1:nrow(datasets)){
  get_weather_summary_raster(datasets[i, 'covYear'], datasets[i, 'covMonth'], 1, 'month',
                             datasets[i, 'lag'], 'pr', stat = 'mean')
  layer_raster <- raster::raster(paste(work_dir,'data/pr/test.pr.tif',sep='/'))
  layer_image <- as.im.RasterLayer(layer_raster)
  df <- fires %>% 
    dplyr::filter(FIRE_YEAR == datasets[i, 'year'],
                  month == datasets[i, 'month'])
  ...
  result <- data.frame(lag = datasets[i, 'lag'],
                       ...
                       wald.pValue = 2*pnorm(-abs(modsum$coefs.SE.CI$Zval[2])))
  results <- rbind(results, result)
  file.remove(paste(work_dir,'data/pr/test.pr.tif',sep = '/'))
  message(paste0(round(i/nrow(datasets), 2)*100, '% done'))
}
write.csv(results, paste(work_dir, 'results/IPPP_byMonthYear_cumPRCP.csv', sep='/'), row.names = F)
